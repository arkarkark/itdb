...
class FormatUtil:
    ...
    def getUrl(self, url):
        urllen = len(url)
        for key in list(self.remapping.keys()):
    ...
    def getLocation(self, base):
        return urllib.parse.unquote_plus(self.getUrl(base))
...
class iTunesDbToHtml:
    def __init__(self, config):
        ...
        for option in config.options("remapping"):
            if option[0] == "s":
                doption = "d" + option[1:]
                remapping[config.get("remapping", option)] = config.get(
                    "remapping", doption
                )
    ...
    def getStarsFromDb(self, thing, where):
        ...
        sql = (
            ...
            "GROUP BY Rating ORDER BY Rating" % (self.userId, where)
        )
        self.cursor.execute(sql)
    ...
    """
        ...
        if where:
            ...
            logging.debug(
                "Writing: %s for %d Tracks", os.path.basename(filename), len(tracks)
            )
    ...
    def WriteStats(self):
        ...
        totals = {}
        totals["track"] = self.getSqlInt(
            "SELECT COUNT(*) FROM tracks " "WHERE User_ID=%d" % self.userId
        )
        totals["album"] = self.getSqlInt(
            "SELECT COUNT(DISTINCT(Album)) "
            "FROM tracks "
            "WHERE User_ID=%d" % self.userId
        )
        totals["artist"] = self.getSqlInt(
        ...
        for x in range(0, 6):
            if totals["track"] > 0:
                ratingspercent.append(
                    "%d%%"
                    % (((ratings[x] if x in ratings else 0) * 100) / totals["track"])
                )
            else:
                ratingspercent.append("  ")

        self.genres = self.getGenresData()
        self.genres.sort(key=lambda x: x.name)
        ...
        self.statsTemplate.types = ["overview", "genre", "artist", "playlist", "album"]
        self.statsTemplate.typesdict = {
            "genre": self.genres,
    ...
    def getPlaylistStats(self, thing, playlistId):
        ...
        sql = (
            ...
            "GROUP BY Stars" % (self.userId, playlistId)
        )
        self.cursor.execute(sql)
    ...
    def writePlaylist(self, thing):
        ...
        playlist_id = self.getSqlInt(
            "SELECT Playlist_ID FROM playlists WHERE "
            "User_ID = %d AND Name = '%s' "
            % (self.userId, MySQLdb.escape_string(thing.name))
        )
    ...
    def getPlaylist(self, thing, playlist_id):
        ...
        sql = (
            ...
            "AND tracks.User_ID = %d" % (playlist_id, self.userId)
        )
        ...
        filename = self.writeTrackList("Playlists: %s" % htmlname, thing, tracks=arr)
    ...
    def writeThing(self, thing):
        ...
        filename = self.writeTrackList(
            "%(column)s: %(htmlname)s" % locals(), thing, where=where
        )
        return filename
    ...
    def GetThingFilename(self, thing):
        ...
        if thing.num_stars is None:
            ...
            return os.path.join(
                tdir, thing.filename + ("-%d" % thing.num_stars) + thing.extension
            )
...
def CgiMain(it):
  ...
  """
    if config.has_option("html", "exceptions") and config.getboolean(
        "html", "exceptions"
    ):
        cgitb.enable()
